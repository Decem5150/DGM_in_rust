...
impl<'a> SpatialDisc<'_> {
    ...
    pub fn integrate_over_edges(&self, mut residuals: ArrayViewMut<f64, Ix3>) {
        let nbasis = self.solver_param.number_of_basis_functions;
        ...
        for edge in self.mesh.edges.iter() {
            ...
            self.compute_edges_values(edge, left_values_gps.view_mut(), right_values_gps.view_mut());
            for igp in 0..ngp {
    ...
    pub fn compute_edges_values(&self, edge: &Edge<'a>, mut left_value_gp: ArrayViewMut<f64, Ix2>, mut right_value_gp: ArrayViewMut<f64, Ix2>) {
        let nbasis = self.solver_param.number_of_basis_functions;
        let ngp = self.solver_param.number_of_edge_gp;
        let neq = self.solver_param.number_of_equations;
